...
chrome.runtime.onInstalled.addListener(async () => {
  // Set default blocked sites on first install
...
async function updateBlockingRules(): Promise<void> {
  const data = await chrome.storage.sync.get(['blockedSites', 'isEnabled']) as StorageData & { isEnabled?: boolean };
  ...
  // Create new rules
  const rules: chrome.declarativeNetRequest.Rule[] = blockedSites.map((domain, index) => ({
    id: index + 1,
    priority: 1,
    action: {
